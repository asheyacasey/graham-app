...
import { buyer_payout_request, get_buyer_security_fee_available_payouts, get_seller_payouts_available, seller_payout_request } from '@/services/payout-request.services';
...
const PayoutSecurityFesses = () => {
    ...
    const GetPayoutRequests = useCallback(async () => {
        try {
            ...
            const { data } = await get_buyer_security_fee_available_payouts({ page: page, limit })
...
export default PayoutSecurityFesses
...
const TableRow = ({ data, remove_order }: { data: IData, remove_order: (order_id: string) => void }) => {
    ...
    const calculateSellerPayoutAmmount = () => {
        return data.refunded_amount ? data.refunded_amount : 0
    }
    const handlePayoutRequest = async (account_id: string) => {
        try {
            ...
            // payout_amount
            // user_account
            await buyer_payout_request({
                order_id: data._id,
                payout_amount: calculateSellerPayoutAmmount(),
                payout_type: PAYOUT_TYPE_ENUM.SECURITY_AMOUNT,
